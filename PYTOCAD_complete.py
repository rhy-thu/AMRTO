...
def write_3dm():
    os.system('start ""/d "D:\\CODE_AMRTO" /wait "json2on.exe" "data/SINGLE_HM_NURBS_surface_total.json"')
...
def NURBSFIT_single(judge_num_NURBS, num_patches, X, size_u, size_v, num_ctrlpts_u,
    ...
    if export_single_json == 1:
        surface.delta = 0.04
        exchange.export_json(surface, 'data/SINGLE_HM_NURBS_surface_' + str(judge_num_NURBS) + '.json')
    elif export_single_json == 2:
        surface.delta = k_delta
        exchange.export_json(surface, 'data/SINGLE_HM_NURBS_surface_' + str(judge_num_NURBS) + '.json')
    if export_single_obj == 1:
        surface.delta = 0.04
        exchange.export_obj(surface, 'data/SINGLE_HM_NURBS_surface_obj_' + str(judge_num_NURBS) + '.obj')
...
def CAD_reconstruction(number_quad,simple_vertices_result, pts_vt_result, simple_patch_index_result, quad_vt_result):
    ...
    k_type_gemodl = 0.5  # (kt) The coefficient of the number of control points in the single parameter direction
    ...
    k_sample = 1.0  # (ks) The specified sampling rate coefficient when the remeshing is performed by the
    ...
    file_path = 'data'  # The relative path of reading and storing files
...
if __name__ == '__main__':
    ...
    number_quad_list = list(range(0, 184, 1))  # The scope of the reconstructed model
    ...
    file_name_result = 'data/result_IM_5.19k_TTTTTT.obj'  # A file containing texture coordinates and packaging textures derived from the generalized motorcycle graph method
    ...
    if export_total_json == 1:
        ...
        exchange.export_json(surface_total, 'data/SINGLE_HM_NURBS_surface_total.json')
