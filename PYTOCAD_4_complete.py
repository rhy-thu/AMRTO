...
def write_3dm():
    os.system('start ""/d "D:\\CODE_AMRTO" /wait "json2on.exe" "data_4/SINGLE_HM_NURBS_surface_total.json"')
...
def NURBSFIT_single(judge_num_NURBS, num_patches, X, size_u, size_v, num_ctrlpts_u,
    ...
    if export_single_json == 1:
        surface.delta = 0.04
        exchange.export_json(surface, 'data_4/SINGLE_HM_NURBS_surface_' + str(judge_num_NURBS) + '.json')
    elif export_single_json == 2:
        surface.delta = k_delta
        exchange.export_json(surface, 'data_4/SINGLE_HM_NURBS_surface_' + str(judge_num_NURBS) + '.json')
    if export_single_obj == 1:
        surface.delta = 0.04
        exchange.export_obj(surface, 'data_4/SINGLE_HM_NURBS_surface_obj_' + str(judge_num_NURBS) + '.obj')
...
def CAD_reconstruction(number_quad, simple_vertices_result, pts_vt_result, simple_patch_index_result, quad_vt_result):
    ...
    file_path = 'data_4'  # The relative path of reading and storing files
...
if __name__ == '__main__':
    ...
    number_quad_list = list(range(0, 81, 1))  # The scope of the reconstructed model
    file_name_result = 'data_4/result_cantilever_undeformed_quad_rhino.obj'  # A file containing texture coordinates and packaging textures derived from the generalized motorcycle graph method
    ...
    #surface = list(CAD_reconstruction(i) for i in number_quad_list)
    ...
    if export_total_json == 1:
        ...
        exchange.export_json(surface_total, 'data_4/SINGLE_HM_NURBS_surface_total.json')
