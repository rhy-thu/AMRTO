...
def write_3dm():
    os.system('start ""/d "D:\\CODE_AMRTO" /wait "json2on.exe" "data_smooth/SINGLE_HM_NURBS_surface_total.json"')
...
def CAD_reconstruction(number_quad,simple_vertices_result, pts_vt_result, simple_patch_index_result, quad_vt_result):
    k_type_gemodl = 1.0  # (kt) The coefficient of the number of control points in the single parameter direction
    ...
    file_path = 'data_smooth'  # The relative path of reading and storing files
...
if __name__ == '__main__':
    ...
    number_quad_list = list(range(0, 159, 1))  # The scope of the reconstructed model
    number_quad_list = np.unique(number_quad_list)
    file_name_result = 'data_smooth/smooth_060.obj'  # A file containing texture coordinates and packaging textures derived from the generalized motorcycle graph method
    ...
    exchange.export_json(surface_total, 'data_smooth/SINGLE_HM_NURBS_surface_total.json')
    write_3dm()
